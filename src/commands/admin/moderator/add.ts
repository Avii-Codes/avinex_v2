...
const command: HybridCommand = {
    name: 'add',
    description: 'Add a moderator (user or role)',
    type: 'both',
    level: 'Admin',
    ...
    async run(ctx) {
        ...
        if (user) {
            if (guildData.settings.moderation.moderatorUsers.includes(targetId)) {
                const container = new Container()
                    .addText(`⚠️ ${targetType} **${targetName}** is already a moderator.`);
                await ctx.reply({ components: [container], flags: [MessageFlags.IsComponentsV2] });
                return;
            }
            guildData.settings.moderation.moderatorUsers.push(targetId);
        } else {
            if (guildData.settings.moderation.moderatorRoles.includes(targetId)) {
                const container = new Container()
                    .addText(`⚠️ ${targetType} **${targetName}** is already a moderator.`);
                await ctx.reply({ components: [container], flags: [MessageFlags.IsComponentsV2] });
                return;
            }
            guildData.settings.moderation.moderatorRoles.push(targetId);
        }

        await guildData.save();
        const container = new Container()
            .addText(`✅ Added ${targetType} ${targetMention} as a moderator.`);
        await ctx.reply({ components: [container], flags: [MessageFlags.IsComponentsV2] });
    }
};

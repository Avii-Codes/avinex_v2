...
const command = {
    name: 'enable',
    description: 'Enable an entire category',
    ...
    async run(ctx) {
        ...
        const container = new components_1.Container()
            .addText(`${emoji_1.Status.warning} **Confirm Category Enable**`)
            .addSeparator({ spacing: 'small', divider: true })
            .addText(`This will enable **${count} commands** in the \`${category}\` category.`)
            .addDivider()
            .addActionRow({
            buttons: [
                {
                    label: 'Confirm',
                    customId: `config:category:enable:${category}`,
                    style: 'success',
                    emoji: '✅'
                },
                {
